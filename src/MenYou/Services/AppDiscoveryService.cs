...
public sealed class AppDiscoveryService : IAppDiscoveryService
{
    ...
    private void MergeUwp(MenuFolder root, List<AppEntry> flat, IReadOnlyList<UwpAppEnumerator.UwpApp> uwp)
    {
        ...
        {
            // Skip uninstall-style entries, mirroring FromShortcut's filter.
            // shell:AppsFolder lists AUMID-registered Win32 uninstallers too
            // ("Uninstall wxNotepad++", "Uninstall Node.js", VirtualBox's bare
            // "Uninstall") and without this they surfaced as launchable tiles.
            if (name.Contains("uninstall", StringComparison.OrdinalIgnoreCase))
                continue;

            // Ghost check: for unregistered Win32 apps the shell reports the
            // raw exe path as the "AUMID", and its resolver cache keeps
            // listing such apps for a while AFTER they're uninstalled. A scan
            // running in that window used to ingest the ghost — and persist
            // it into the discovery cache, where it survived indefinitely
            // (the ghost's later disappearance from AppsFolder changes no
            // file, so the fingerprint never flips). Launching one routed
            // explorer.exe to an invalid shell:AppsFolder item, which opens
            // the Documents folder instead. Real AUMIDs (Package!App,
            // Microsoft.AutoGenerated.{…}, {KNOWNFOLDERID}\…) aren't
            // drive-rooted paths and skip this check.
            if (Path.IsPathFullyQualified(aumid) && !File.Exists(aumid))
                continue;
    ...
    private static AppEntry? FromShortcut(string lnkPath)
    {
        ...
        // Skip uninstall-style entries. The name checks run BEFORE the
        // info-null early return: a shortcut whose ShellLink read fails used
        // to bypass the filter entirely and surface as a launchable
        // "Uninstall …" tile.
