...
**Single instance** — MenYou runs one instance per session. `Program.Main` calls `SingleInstance.TryAcquire()` before any startup work: the first instance creates a session-local named **mutex** (`Local\MenYou.SingleInstance.Mutex`) and parks a background thread on a named auto-reset **event** (`…Show`). A later launch opens that event, rings it, and exits — so it never spins up a duplicate tray icon / hotkey / window. The running instance's listener wakes and calls `ShowStartMenu()` (the same idempotent reveal the tray Open verb uses), so re-launching MenYou — or a stray second logon-task firing — *surfaces the menu* rather than doing nothing. The mutex releases automatically on process exit (even a hard crash, so there's no stale lock), and the `Local\` namespace keeps fast-user-switched sessions independent — a system-wide `Global\` guard would wrongly block the second signed-in user. The listener is wired up in `App.SetupSingleInstanceShowListener`.

> [!NOTE]
> The guard is **path-blind** — one fixed mutex name, so *any* MenYou binary claims it (dev build, portable extract, an old folder left by a moved install). That normally doesn't matter, but it bit upgrades: Restart Manager only closes processes holding files in the install dir, so a MenYou running from anywhere else survived the upgrade, kept the mutex, and the freshly installed exe handed off to that **stale** process and exited — leaving the user on the old build, reporting the old version, with no sign the update hadn't taken. The installer now sweeps those strays in `PrepareToInstall` (`CloseStrayMenYou` in `installer/inno/menyou.iss`: WMI-enumerate `MenYou.exe`, terminate the ones whose path is outside the install dir, and deliberately leave the in-dir one to Restart Manager so it is closed *and relaunched* gracefully). The deeper fix — making the newcomer compare versions and take over rather than always deferring — is not done; the same shadowing is still reachable without an installer (launching the installed exe while a portable copy runs).

**Startup performance** — full write-up and measurements in [`OPTIMIZATION.md`](OPTIMIZATION.md).
- *Autostart via logon task* — `Win32AutostartService` registers a per-user logon-triggered Scheduled Task (`InteractiveToken` / `LeastPrivilege`, `PT1S` delay) rather than an `HKCU\Run` value, because Windows throttles Run-key/Startup-folder autostarts ~15 s after sign-in. The task is exempt, so MenYou launches ~1 s after the desktop (a ~15 s → ~1 s cold-start win). Falls back to the Run-key (plus a zeroed `StartupDelayInMSec`) where task creation is blocked; a one-time, self-healing migration moves existing installs over and only marks itself done once autostart is verifiably in place. On uninstall the installer removes the task (and any legacy `HKCU\Run` value) so it can't linger and fire at each sign-in after MenYou is gone.
- *App-discovery cache* — `AppDiscoveryService` persists the resolved app list to `%AppData%\MenYou\discovery-cache.json` (`DiscoveryCache`). On a cold start it paints from that snapshot — a plain file read, no shell COM, so it sidesteps the ~5× COM slowdown while Windows is still bringing up the shell. Because that path is COM-free, `PreloadFromCacheAsync` runs it *eagerly at the very start of startup* (≈150 ms in) instead of waiting for the idle warm-up, so the menu's data is ready almost immediately. A cheap, COM-free filesystem fingerprint (paths + mtimes + sizes) tells the backstop whether the snapshot is *current* — but since 0.8.7 the paint happens either way (**stale-while-revalidate**: a stale snapshot is at most one app-shaped diff behind, and a ~20 s empty menu was the alternative). The background live scan (after a settle delay) swaps in corrections, fires `Refreshed` only if the list actually changed, and re-persists the cache + fingerprint on a stale paint or Start-Menu change so the next boot is a hit. Scans whose UWP enumeration comes back empty are treated as degraded and never replace or persist over good data. Toggle: Settings → Developer → *App-list cache*.
...
- *Parallel discovery + icon fill* — the `.lnk` walk parses shortcuts across cores (`Parallel.For`) and overlaps the `shell:AppsFolder` UWP enumeration; the cog→real-icon tile fill likewise batches all extractions through `Parallel.ForEachAsync` instead of awaiting them one at a time, with each icon posted to the UI as it lands.
- *"Ready" balloon* — once the tray + hotkey are live, a one-time tray balloon confirms MenYou is up and teaches the **Shift+Win** hotkey. It shows on a brand-new profile (no `settings.json` yet) **and after every install/update**: the Inno installer drops a one-shot `HKCU\Software\MenYou\ShowReadyBalloon` marker that `ConsumeReadyMarker` reads and clears on the next launch, so reinstalls and upgrades still get it even though `settings.json` survives uninstall. (A GDI first-run splash covering the cold install load shipped in 0.7.0 but was removed in 0.8.0 — it didn't reliably paint on a fresh install.)
...
- *Per-app JumpLists* — `JumpListReader` ports Open-Shell's reverse- engineered COM: `IAutomaticDestinationList` (Recent/Frequent), `IDestinationList` (Tasks/custom destinations), and `IApplicationResolver` (the implicit AUMID for plain `.lnk`s); `JumpListReader.ResolveKey` derives the destination-list key from an AUMID / `.lnk` / path. They surface in every app context menu, built dynamically so the app's published **Tasks** ("New window", "New private window", …) + **Recent** files appear inline below the standard verbs + Pin/Unpin, streaming into the open menu as the JumpList loads. Two builders share that shape: `SearchResultsList` (search results + its detail panel) and the **`AppContextMenuBehavior`** attached property — one `Enable="True"` on each app tile/row drives the Win 11 Pinned / Recent / All-Programs grids, the Win 7 pinned strip + recent list, the All-Programs tree, and the folder flyout, replacing five divergent declarative menus with one definition. The number of **Recent** files listed in those context menus is capped by *Settings → Recent files in context menu* (`ContextMenuRecentCount`, default 8; 0 hides them); the Win 7 / Classic side detail panel is intentionally exempt and shows the full recent set.
...
**In-app updates** — `GitHubUpdateService` backs Settings → **Check for updates**: it takes the current version from the **running assembly** (Inno's `{AppId}_is1` uninstall key is consulted only for its *presence*, to tell an installed build from a dev/portable run — its `DisplayVersion` goes stale across in-place upgrades, which once made the updater re-offer the same release forever), queries `releases/latest`, and downloads + silently runs the newer installer for this install's variant. Inno's Restart Manager closes and relaunches MenYou around the file swap. The **About** button (and tray entry) open the project page; the **version label** beside it is a link to `CHANGELOG.md` at that version's tag (`GitHubUpdateService.ChangelogUrl`).
...
**On uninstall** this `%AppData%\MenYou\` folder is left intact — your settings, custom themes, and cache survive a reinstall. The uninstaller removes only what the app and installer added elsewhere: the autostart logon task and any legacy `HKCU\Run` value (`CurUninstallStepChanged`), the `HKCU\Software\MenYou` key holding the one-shot ready-balloon marker (`uninsdeletekey`), and the bridge shadow-copy under `%LocalAppData%\MenYou\bridge\` (`[UninstallDelete]`).
