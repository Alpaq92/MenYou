...
- *First-run "ready" balloon* — on first run (no `settings.json` yet), once the tray + hotkey are live a one-time tray balloon confirms MenYou is up and teaches the **Shift+Win** hotkey. (A GDI first-run splash covering the cold install load shipped in 0.7.0 but was removed in 0.8.0 — it didn't reliably paint on a fresh install.)
