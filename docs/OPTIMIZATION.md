...
- **It was the only artifact Defender kept flagging.** Every `Trojan:Win32/Wacatac.B!ml` detection MenYou has ever had — 0.9.15 and 0.9.19 — was on `MenYou-fd-Setup`; the self-contained and arm64 installers have never tripped once, including on manual downloads of the same release. The verdict is not reproducible offline: the payload files, a locally built FD installer, and that installer with a GitHub Mark-of-the-Web all scan clean under `MpCmdRun`. It is a cloud call made at download time against a zero-prevalence unsigned binary — and FD was the variant with the least of everything the model rewards. Its payload is almost entirely unsigned MenYou code, where the self-contained builds are dominated by Microsoft-signed runtime DLLs; and it was deliberately excluded from winget/Chocolatey, so it never accumulated channel prevalence either.
